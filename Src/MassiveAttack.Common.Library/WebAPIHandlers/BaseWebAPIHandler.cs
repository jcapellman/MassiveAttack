...
namespace MassiveAttack.Common.Library.WebAPIHandlers {
    public class BaseWebAPIHandler {
        ...
        protected HandlerConstructorItem Wrapper;

        public BaseWebAPIHandler(HandlerConstructorItem constructorItem)
        {
            Wrapper = constructorItem;
        ...
        public async Task<TK> PostAsync<T, TK>(string relativeURL, T objectValue)
        {
            var result = await GetHttpClient().PostAsync(BuildUrl(relativeURL), new MultipartContent());
